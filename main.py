...
async def get_all_expenses():
    """Retrieve all expenses."""
    try:
        cursor = await expenses_collection.find()
        data = await cursor.to_list(None)
...
#     filter_query = {}
#     if date: filter_query["date"] = convert_date(date)
#     if amount is not None: filter_query["amount"] = amount
#     if category: filter_query["category"] = category
#     if subcategory: filter_query["subcategory"] = subcategory
#     if note: filter_query["note"] = note
#     if payment_method: filter_query["payment_method"] = payment_method
...
#     target_id = matches[0]["_id"]
#     update_fields = {}
#     if new_date: update_fields["date"] = convert_date(new_date)
#     if new_amount is not None: update_fields["amount"] = new_amount
#     if new_category: update_fields["category"] = new_category
#     if new_subcategory: update_fields["subcategory"] = new_subcategory
#     if new_note: update_fields["note"] = new_note
#     if new_payment_method: update_fields["payment_method"] = new_payment_method

#     if not update_fields:
#         return {"status": "error", "message": "No update fields provided."}

#     await expenses_collection.update_one({"_id": target_id}, {"$set": update_fields})
